...
#' @param date_format Format to use if "Date" scales are required. If NULL
...
#'   `date_format` is NULL.
#' @param sep Seperator to use for grouped date labels.
#' @param interval An integer indicating the (fixed) number of months used for
#'   grouping; defaults to 1.
#' @param origin The date to anchor the intervals from. If NULL (default) the
#'   earliest date in the vector will be used.
#'
#' @return A scale for use with ggplot2.
#' @export
scale_x_grate_month <- function(n.breaks = 6, date_format = NULL,
  ...
  if (missing(interval)) {
    ...
    if (is.null(interval)) {
      stop("Please specify the `interval` of the grate_month input", call. = FALSE)
    }
  }
  ...
  if (missing(origin)) {
    ...
    if (is.null(origin)) {
      stop("Please specify the `origin` of the grate_month input", call. = FALSE)
    }
  }
...
grate_month_trans <- function(n.breaks, date_format, bounds_format, sep, interval, origin) {

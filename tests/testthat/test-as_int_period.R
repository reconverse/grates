...
test_that("as.character.period works as expected", {
  dat <- 1
  res <- as.character(as_period(dat, interval = 2))
  expect_equal(res, "1 - 2")
})

test_that("as.list.period works as expected", {
  dat <- as_period(c(a = 1, b = 8), interval = 2)
  res <- list(
    a = as_period(1, interval = 2),
    b = as_period(8, origin = 1, interval = 2))
...
test_that("is_period/grate works", {
  dat <- as_period(as.integer(Sys.Date()), interval = 2)
...
test_that("subsetting works", {
  ...
  expect_equal(
    dat[[2]],
    as_period(x + 31, interval = 31, origin = as.integer(as.Date("2021-01-15")))
  )

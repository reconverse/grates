...
**yearweek** objects (of class `<grates_yearweek>`) are stored as the number of
weeks (starting at 0L) from the date of the `firstday` nearest the Unix Epoch
(1970-01-01). Put more simply, the number of seven day periods from:
...
They can be constructed directly from integers via the `yearweek()` function but
it is generally easier to use the `as_yearweek()` generic. `as_yearweek()` takes
two arguments; `x`, the vector (normally a Date or POSIXt) you wish to group,
...
**period** objects (of class`<grates_period>`) are stored as the integer number,
starting at 0L, of periods since the Unix Epoch (1970-01-01) and a specified
offset. Here periods are taken to mean groupings of `n` consecutive days.

Like with yearweek objects, period objects can be constructed directly via a 
call to `period()` but more easily via the `as_period()` coercion function.
...
# calculate the total number for across 14 day periods 0 offset.
...
**yearmonth**, **yearquarter** and **year** objects (of class
`<grates_yearmonth>`, `<grates_yearquarter>` and `<grates_year>` respectively)
are stored as the number of integer months/quarters/years (starting at 0L)
since the Unix Epoch (1970-01-01). Precision is only to the level of the given
grouping (i.e. the day of the month/quarter/year is always dropped).

As with all grates objects they they can be constructed directly from integers
via the `yearmonth()`, `yearquarter()` and `year()`  functions but it is often
easier to use the `as_yearmonth()`, `as_yearquarter()` and `as_year()` coercion
functions.
...
**month** objects (class `<grates_month>`) are stored as the position, starting
at 0, of n-month groups since the Unix Epoch (1970-01-01). Here n-months is
taken to mean a 'grouping of n consecutive months'.

`<grates_month>` objects can be constructed directly from integers via the
`month()` function and throuh coercion via the `as_month()` function.
